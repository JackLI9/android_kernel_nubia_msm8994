...
static DEVICE_ATTR(update_fw0, S_IWUSR /*| S_IWGRP | S_IWOTH */, NULL, mxt_update_fw_store);
static DEVICE_ATTR(update_cfg0, S_IWUSR/* | S_IWGRP | S_IWOTH */, NULL, mxt_update_cfg_store);
static DEVICE_ATTR(debug_v2_enable, S_IWUSR/*S_IWUSR | S_IRUSR*/, NULL, mxt_debug_v2_enable_store);
...
/*** ZTEMT start ,20150123***/
static DEVICE_ATTR(ic_detect, 0444, mxt_ic_detect_show,NULL);
static DEVICE_ATTR(manual_cali, 0444, mxt_manual_cali_show,NULL);
static DEVICE_ATTR(easy_wakeup_gesture, S_IWUSR, NULL, mxt_easy_wakeup_gesture_store);//mxt_plugin_wakeup_gesture_show
...
static DEVICE_ATTR(depth, S_IWUSR | S_IRUSR, mxt_irq_depth_show, 
			mxt_irq_depth_store);
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
static DEVICE_ATTR(plugin, S_IWUSR /*S_IWUSR | S_IRUSR*/, mxt_plugin_show,
			mxt_plugin_store);
...
/*ZTEMT start add for panel without rim*/
	...
	for (id = 0; id < num_mt_slots; id++) {
		...
		if (test_flag_8bit(MXT_T100_TCHAUX_AMPL, &data->tchcfg[MXT_T100_TCHAUX]))
			input_report_abs(input_rim_dev, ABS_MT_PRESSURE,
					 0);
