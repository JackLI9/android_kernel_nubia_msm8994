...
#define CONFIG_MXT_CAL_WORKAROUND
#define CONFIG_MXT_AC_WORKAROUND
#define CONFIG_MXT_PI_WORKAROUND
...
//#define CONFIG_CHIP_540S
#define CONFIG_MXT_UPDATE_BY_OBJECT

#if (defined(CONFIG_MXT_CAL_WORKAROUND) || \
	defined(CONFIG_MXT_AC_WORKAROUND) || \
	defined(CONFIG_MXT_PI_WORKAROUND) || \
...
struct t104_config {
	struct sc_obj_config x;
	struct sc_obj_config y;
}__packed;
...
struct plugin_cal{
	...
	int (*hook_t100)(struct plugin_cal *p, int id, int x, int y, struct ext_info *in);
	int (*hook_t100_scraux)(struct plugin_cal *p, struct scr_info *in);
...
struct plugin_proci{
	...
	int (*hook_t81)(struct plugin_proci *p, u8 *msg, unsigned long pl_flag);
...
struct plugin_misc{
	...
	//client
	int (*init)(struct plugin_misc *p, unsigned long pl_flag);
