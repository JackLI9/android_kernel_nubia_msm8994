...
static DEVICE_ATTR(ram_data, 0664, ram_data_show, ram_data_store);
static DEVICE_ATTR(ram_load, 0664, ram_load_show, ram_load_store);
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	struct ak49xx *ak49xx = dev_get_drvdata(codec->dev->parent);
	int internal_mic_gain = ucontrol->value.integer.value[0];
	u8	val[12];
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct ak49xx *ak49xx = dev_get_drvdata(codec->dev->parent);
	int internal_ahpf_sensitivity = ucontrol->value.integer.value[0];
	u8	val[12];
