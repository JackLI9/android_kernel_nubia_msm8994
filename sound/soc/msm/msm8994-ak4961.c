	...
	struct snd_ctl_elem_value *ucontrol)
{
    ...
    if(headphone_switch_gpio >= 0) {
        pr_debug("%s: gpio value is %d\n",__func__,gpio_get_value(headphone_switch_gpio));
...
static int tps_5v_switch_gpio_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{

    if(tps_enable_gpio >= 0) {
        pr_debug("%s: gpio value is %d\n",__func__,gpio_get_value(tps_enable_gpio));
        ucontrol->value.integer.value[0] = gpio_get_value(tps_enable_gpio);
    }
    return 0;
}

static int tps_5v_switch_gpio_put(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
    switch (ucontrol->value.integer.value[0]) {
        case 1:
					...
					bool dapm)
{
	...
	if (enable) {
		...
		ak4961_mclk_enable(codec, 1, dapm);
	} else {
		if (clk_users > 0) {
			clk_users--;
			if (clk_users == 0) {
				ak4961_mclk_enable(codec, 0, dapm);
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("TPS 5V Switch",msm_snd_enum[9],
			tps_5v_switch_gpio_get, tps_5v_switch_gpio_put),
#endif
};
...
static int msm8994_ak4961_codec_up(struct snd_soc_codec *codec)
{
	int err;
		...
		enum ak49xx_codec_event codec_event)
{
	...
	case AK49XX_CODEC_EVENT_CODEC_UP:
		return msm8994_ak4961_codec_up(codec);
	default:
...
/*
	config_data = ak4961_get_afe_config(codec, AFE_AANC_VERSION);
	...
	config_data = ak4961_get_afe_config(codec,
	...
	config_data = ak4961_get_afe_config(codec, AFE_CLIP_BANK_SEL);
	if (config_data) {
...
*/
	...
	if (!adsp_state_notifier) {
		pr_err("%s: Failed to register adsp state notifier\n",
		       __func__);
		err = -EFAULT;
//		ak4961_hs_detect_exit(codec);
		goto out;
	}

	ak4961_event_register(msm8994_codec_event_cb, rtd->codec);
...
static int msm8994_prepare_codec_mclk(struct snd_soc_card *card)
{
	...
	int ret;
	if (pdata->mclk_gpio) {
		ret = gpio_request(pdata->mclk_gpio, "AK4961_CODEC_PMIC_MCLK");
		if (ret) {
			dev_err(card->dev,
...
static int msm8994_prepare_us_euro(struct snd_soc_card *card)
{
	...
	if (pdata->us_euro_gpio >= 0) {
		dev_dbg(card->dev, "%s: us_euro gpio request %d", __func__,
			pdata->us_euro_gpio);
		ret = gpio_request(pdata->us_euro_gpio, "AK4961_CODEC_US_EURO");
		if (ret) {
			dev_err(card->dev,
...
static int msm8994_asoc_machine_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node,
			"qcom,ak4961-mclk-clk-freq", &pdata->mclk_freq);
	if (ret) {
		dev_err(&pdev->dev,
			"Looking up %s property in node %s failed, err%d\n",
			"qcom,ak4961-mclk-clk-freq",
